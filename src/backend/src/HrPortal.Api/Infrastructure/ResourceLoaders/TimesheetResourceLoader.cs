...
internal sealed class TimesheetResourceLoader : IEndpointResourceLoader
{
    private readonly HrPortalDbContext _dbContext;
    private readonly ITenantContextAccessor _accessor;

    public TimesheetResourceLoader(HrPortalDbContext dbContext, ITenantContextAccessor accessor)
    {
    ...
    public bool CanLoad(HttpContext httpContext) =>
        ResourceLoaderHelpers.MatchesResourcePath(httpContext, "/api/v1/timesheets");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var submission = await _dbContext.Set<TimesheetSubmission>()
            .AsNoTracking()
