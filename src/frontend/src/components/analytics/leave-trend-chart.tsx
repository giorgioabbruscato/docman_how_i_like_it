...
import { useLeaveTrendChart } from '@/hooks/use-analytics';
...
export function LeaveTrendChart({ filters }: LeaveTrendChartProps) {
  const { data, isLoading, isError, error } = useLeaveTrendChart(filters);
  ...
  return (
    <ChartCard
      ...
      isLoading={isLoading}
      isError={isError}
      errorMessage={getApiErrorMessage(error, 'Failed to load leave trend.')}
